...
{
	char tmp[1024];
	mem_copy(tmp, line, strlen(line)); tmp[strlen(line)] = '\r';
	LogOverEXI(tmp, strlen(line)+1);
...
static void mem_init()
{
	...
	void *mem1_heap_ptr = (void *)((u32)SYS_GetArena1Lo());
	void *mem2_heap_ptr = (void *)((u32)SYS_GetArena2Hi()-size[1]);
	SYS_SetArena1Lo(mem1_heap_ptr+size[0]);
...
void *thread_init(void (*threadfunc)(void *), void *u)
{
	...
	LWP_CreateThread(&thread, ExecThread, (void*)&d, NULL, 0, 80);
	return (void*)thread;
#else
	#error not implemented
#endif
...
#elif defined(HW_RVL)
	LWP_JoinThread((lwp_t)thread, NULL);
#else
	#error not implemented
#endif
}
...
#elif defined(HW_RVL)
	mutex_t* mut = (mutex_t*)lock;
	LWP_MutexDestroy(*mut);
#else
	#error not implemented on this platform
#endif
	mem_free(lock);
...
void lock_unlock(LOCK lock)
{
	...
	#elif defined(HW_RVL)
	void semaphore_init(SEMAPHORE *sem) { LWP_SemInit((sem_t*)sem, 0, 0); }
	void semaphore_wait(SEMAPHORE *sem) { LWP_SemWait((sem_t)*sem); }
	void semaphore_signal(SEMAPHORE *sem) { LWP_SemPost((sem_t)*sem); }
	void semaphore_destroy(SEMAPHORE *sem) { LWP_SemDestroy((sem_t)*sem); }
	#else
		#error not implemented on this platform
	#endif
#endif
...
{
	snprintf(path, max, "sd:/apps/ddnet/data/user");
	return 0;
}
